...
function clickyStage(){
  var otherSaveAs = saveAs;
  ...
  var installFinality = function(){
    ...
    $(".poembanner").css({opacity: 1});


  };


    var poem_saved = false;
    $("#poembtnSave").click(_.throttle(function() {
      $("#poembtnSave").remove();
      if(!poem_saved){
        poem_saved = true;
        html2canvas($(".boring"), {
          onrendered: function(canvas) {
            ...
            canvas.toBlob(function(blob) {
              otherSaveAs(blob, "an_poem"+Date.now()+".png");
              otherSaveAs = undefined;
            });
          }
        });
      }
          ...
          if(!saved){
            ...
            html2canvas($(".bubble"), {
                onrendered: function(canvas) {
                    theCanvas = canvas;
                    canvas.toBlob(function(blob) {
                        saveAs(blob, "a_bubble_art"+Date.now()+".png");
                        saveAs = undefined;
